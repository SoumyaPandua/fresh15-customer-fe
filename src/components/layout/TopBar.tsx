    ...
    <header className="sticky top-0 z-40 border-b bg-background/80 glass">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-3 py-3 sm:gap-4 sm:px-6">
        <Link to="/" className="flex shrink-0 items-center gap-2">
          <div className="grid h-9 w-9 place-items-center rounded-xl gradient-primary text-primary-foreground shadow-soft"><span className="text-lg font-black">F</span></div>
          ...
          <PopoverTrigger asChild>
            <button className="flex min-w-0 shrink items-center gap-1.5 rounded-xl px-2 py-1.5 text-left hover:bg-muted">
              ...
              <div className="min-w-0 leading-tight">
                ...
                <div className="truncate text-[11px] text-muted-foreground">{active?.line1 || active?.city || "Choose an address"}</div>
              </div>
          ...
          <PopoverContent align="start" className="w-80 p-2">
            ...
            {active && (
              <div className="mb-2 rounded-lg border border-primary/20 bg-primary/5 p-2 text-xs">
                <div className="font-semibold">{service.isLoading ? "Checking delivery availability…" : service.serviceable ? `${service.etaMinutes ?? ""}${service.etaMinutes ? " min delivery" : "Delivery available"}` : "Delivery unavailable"}</div>
                {service.storeName && <div className="mt-0.5 text-[11px] text-muted-foreground">{service.storeName}{service.storeDistanceKm != null ? ` · ${service.storeDistanceKm.toFixed(1)} km` : ""}</div>}
                {service.serviceable && service.minOrder > 0 && <div className="mt-0.5 text-[11px] text-muted-foreground">Minimum order ₹{service.minOrder}</div>}
              </div>
            )}
            <div className="space-y-1">
              {addresses.map((address) => (
                <button key={address.id} onClick={() => void setActive(address.id)} className={`block w-full rounded-lg p-2 text-left text-sm hover:bg-muted ${address.id === activeAddressId ? "bg-muted" : ""}`}>
        ...
        {!hideSearch && (
          <form className="ml-2 hidden flex-1 md:block" onSubmit={(event) => { event.preventDefault(); void navigate({ to: "/search", search: { q: query } }); }}>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1 sm:gap-2">
          <Link to="/search" className="grid h-9 w-9 place-items-center rounded-full hover:bg-muted md:hidden" aria-label="Search"><Search className="h-4 w-4" /></Link>
          <button onClick={() => setNotifOpen(true)} className="relative hidden h-9 w-9 place-items-center rounded-full hover:bg-muted sm:grid" aria-label="Notifications">
            <Bell className="h-4 w-4" />{notif.unreadCount > 0 && <span className="absolute right-1.5 top-1.5 h-2.5 w-2.5 rounded-full bg-destructive ring-2 ring-background" />}
          </button>
          <Link to="/wishlist" className="relative hidden h-9 w-9 place-items-center rounded-full hover:bg-muted sm:grid" aria-label="Wishlist">
            <Heart className="h-4 w-4" />{wishCount > 0 && <span className="absolute -right-0.5 -top-0.5 grid h-4 min-w-4 place-items-center rounded-full bg-destructive px-1 text-[10px] font-bold text-white">{wishCount}</span>}
          </Link>
          <ThemeToggle />
          <Link to={user || isGuest ? "/profile" : "/auth/login"} className="hidden h-9 w-9 place-items-center rounded-full hover:bg-muted sm:grid" aria-label="Profile"><User className="h-4 w-4" /></Link>
          <Link to="/cart" className="relative inline-flex items-center gap-1.5 rounded-full bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground shadow-soft sm:px-4">
            <ShoppingCart className="h-4 w-4" /><span className="hidden sm:inline">Cart</span>{cartCount > 0 && <span className="ml-0.5 grid h-5 min-w-5 place-items-center rounded-full bg-white/25 px-1 text-[11px] font-bold">{cartCount}</span>}
          </Link>
        </div>
      </div>
