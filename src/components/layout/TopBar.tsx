...
export function TopBar() {
  ...
  const { addresses, activeAddressId, setActive, addressReady } =
    useAddressBook();
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-3 py-3 sm:gap-4 sm:px-6">
        ...
        <Popover>
          ...
          <PopoverContent align="start" className="w-80 p-2">
              ...
              <div className="mb-2 rounded-lg border border-primary/20 bg-primary/5 p-2 text-xs">
                ...
                {service.storeName && (
                  <div className="mt-0.5 text-[11px] text-muted-foreground">
                    {service.storeName}
                    {service.storeDistanceKm != null
                      ? ` · ${service.storeDistanceKm.toFixed(1)} km`
                      : ""}
                  </div>
                )}
              ...
              {addresses.map((address) => (
                <button
                  ...
                  className={`block w-full rounded-lg p-2 text-left text-sm hover:bg-muted ${
                    address.id === activeAddressId ? "bg-muted" : ""
                  }`}
                >
            ...
            <Link
              to="/addresses"
              className="mt-2 block rounded-lg bg-primary/10 p-2 text-center text-sm font-semibold text-primary"
            >
              Manage addresses
            </Link>
          </PopoverContent>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          <Link
            to={user || isGuest ? "/profile" : "/auth/login"}
          ...
          <Link
            to="/cart"
            className="relative inline-flex items-center gap-1.5 rounded-full bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground shadow-soft sm:px-4"
          >
            <ShoppingCart className="h-4 w-4" />
            <span className="hidden sm:inline">Cart</span>
            {cartCount > 0 && (
              <span className="ml-0.5 grid h-5 min-w-5 place-items-center rounded-full bg-white/25 px-1 text-[11px] font-bold">
                {cartCount}
              </span>
            )}
          </Link>
      ...
      <Dialog open={notifOpen} onOpenChange={setNotifOpen}>
        <DialogContent className="max-w-md p-0">
              ...
              notif.notifications.map((item) => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => {
                      if (!item.read) notif.markRead.mutate(item.id);
                  ...
                  >
                    <div className="relative grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-muted text-primary">
          ...
          <div className="flex items-center justify-between border-t px-5 py-3">
            {notif.unreadCount > 0 ? (
              <button
                onClick={() => notif.markAllRead.mutate()}
                className="text-xs font-semibold text-muted-foreground"
                disabled={notif.markAllRead.isPending}
              >
