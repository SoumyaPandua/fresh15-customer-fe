...
export function TopBar() {
  ...
  const { addresses, activeAddressId, setActive } = useAddressBook();
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-3 py-3 sm:gap-4 sm:px-6">
        <Link to="/" className="flex shrink-0 items-center gap-2">
          ...
          <div className="hidden leading-tight sm:block">
            <div className="text-base font-black tracking-tight text-foreground">Fresh15</div>
            ...
            <button className="flex min-w-0 shrink items-center gap-1.5 rounded-xl px-2 py-1.5 text-left hover:bg-muted">
              ...
              <div className="min-w-0 leading-tight">
                <div className="truncate text-xs font-semibold text-foreground sm:text-sm">
                  {active?.label ?? "Deliver to"} · {pincode}
                </div>
                <div className="truncate text-[11px] text-muted-foreground">{active ? `${active.line1}` : city}</div>
              </div>
          ...
          <PopoverContent align="start" className="w-72 p-2">
            ...
            <div className="space-y-1">
              {addresses.map((a) => (
                <button
                  key={a.id}
                  onClick={() => void setActive(a.id).catch(() => undefined)}
                  className={
                    "block w-full rounded-lg p-2 text-left text-sm hover:bg-muted " +
                    (a.id === activeAddressId ? "bg-muted" : "")
                  }
                >
                  <div className="font-semibold">{a.label}</div>
                  <div className="line-clamp-1 text-xs text-muted-foreground">{a.line1}, {a.city}</div>
                </button>
              ))}
            </div>
            <Link to="/addresses" className="mt-2 block rounded-lg bg-primary/10 p-2 text-center text-sm font-semibold text-primary hover:bg-primary/15">
              Manage addresses
            </Link>
          </PopoverContent>
        </Popover>
                ...
                {notif.notifications.map((n) => {
                  ...
                  return (
                    <button
                      key={n.id}
                      onClick={() => {
                        if (!n.read) notif.markRead.mutate(n.id);
                        if (n.orderId) {
                          setNotifOpen(false);
                          void navigate({ to: "/orders/$id", params: { id: n.orderId } });
                        }
                      }}
                      className={
