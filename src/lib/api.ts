...
/**
 * Builds visually distinct home sections from one catalog without repeating
 * the same product across multiple primary sections.
 */
export function buildHomeSections(list: Product[]): HomeSections {
  const byTag = (tag: NonNullable<Product["tags"]>[number]) =>
    list.filter((p) => p.tags?.includes(tag));

  const discounted = [...list].sort(
    (a, b) =>
      (b.mrp - b.price) / (b.mrp || 1) - (a.mrp - a.price) / (a.mrp || 1),
  );
  const topRated = [...list].sort((a, b) => b.rating - a.rating || b.reviews - a.reviews);
  const freshMix = [...list].sort(
    (a, b) => Number(Boolean(b.tags?.includes("seasonal"))) - Number(Boolean(a.tags?.includes("seasonal"))),
  );

  const used = new Set<string>();

  const takeUnique = (candidates: Product[], count: number) => {
  ...
  return {
    flashOffers: takeUnique([...byTag("flash"), ...discounted], 5),
    recommended: takeUnique([...byTag("recommended"), ...topRated], 5),
    bestSellers: takeUnique([...byTag("bestseller"), ...topRated], 8),
    seasonal: takeUnique([...byTag("seasonal"), ...freshMix, ...list], 5),
  };
}
...
function targetToHref(type: BannerTargetType | undefined, value?: string) {
  ...
  switch (type) {
    case "CATEGORY":
      return v ? `/category/${encodeURIComponent(v)}` : "/search";
    case "PRODUCT":
      return v ? `/product/${encodeURIComponent(v)}` : "/search";
    case "SEARCH":
      return `/search${v ? `?q=${encodeURIComponent(v)}` : ""}`;
    case "OFFER":
      return "/search";
    default:
      return "/search";
...
export const api = {
  ...
  async getOffers(placement = "HOME"): Promise<StorefrontOffer[]> {
    try {
      const data = await apiRequest<any[]>(
        `/api/offer/active?placement=${encodeURIComponent(placement)}`,
      );
      return (Array.isArray(data) ? data : [])
        .map((o: any) => ({
  ...
  async getProducts(filter?: ProductQuery): Promise<Product[]> {
    return withCatalogFallback(
      async () => catalogApi.getProducts(filter),
  ...
  async validateCoupon(
    code: string,
    subtotal: number,
  ): Promise<{ coupon?: Coupon; discount: number; error?: string }> {
    await delay(400);
  ...
  async checkPincode(pincode: string) {
    await delay(400);
    return {
      serviceable: /^\d{6}$/.test(pincode) && pincode.startsWith("4"),
      etaMinutes: 12,
    };
  },
